...
export default function Blajeni() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        {['all', ...CATEGORIES].map((c) => (
          <button key={c} onClick={() => setCategory(c)}
            className={`rounded-full px-3 py-1 text-xs capitalize ${category === c ? 'bg-accent text-white' : 'bg-surface-2 text-gray-400'}`}>
            {c}
          </button>
        ))}
                ...
                <Card key={t.id} className="!p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className={`text-sm font-medium ${t.status === 'done' ? 'text-gray-500 line-through' : 'text-gray-100'}`}>
                        {t.title}
                      </p>
                      {t.description && <p className="mt-0.5 line-clamp-2 text-xs text-gray-500">{t.description}</p>}
                      <div className="mt-1.5 flex gap-1.5">
                        <span className="rounded bg-surface-2 px-1.5 py-0.5 text-[10px] capitalize text-gray-500">{t.category}</span>
                        <span className={`rounded px-1.5 py-0.5 text-[10px] uppercase ${
                          t.priority === 'high' ? 'bg-red-500/15 text-red-300' :
                          t.priority === 'low' ? 'bg-gray-500/15 text-gray-400' : 'bg-yellow-500/15 text-yellow-300'
                        }`}>{t.priority}</span>
                      </div>
                    </div>
                  ...
                  <div className="mt-2 flex justify-between">
                    <button disabled={colIdx === 0} onClick={() => move(t, -1)}
                      className="text-gray-600 hover:text-gray-300 disabled:opacity-20"><ChevronLeft size={16} /></button>
                    <button disabled={colIdx === 2} onClick={() => move(t, 1)}
                      className="text-gray-600 hover:text-gray-300 disabled:opacity-20"><ChevronRight size={16} /></button>
                  </div>
                </Card>
              ))}
            </div>
      ...
      <div>
        <h3 className="mb-2 text-sm font-semibold text-gray-100">Notes</h3>
        {(notes.data || []).length === 0 ? (
          <Card className="py-6 text-center text-sm text-gray-500">No notes yet.</Card>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {notes.data.map((n) => (
              <Card key={n.id}>
                <h4 className="text-sm font-medium text-gray-100">{n.title}</h4>
                <p className="mt-1 line-clamp-4 whitespace-pre-wrap text-xs text-gray-400">{n.content}</p>
                <span className="mt-2 inline-block rounded bg-surface-2 px-1.5 py-0.5 text-[10px] capitalize text-gray-500">{n.category}</span>
              </Card>
            ))}
          </div>
        )}
